...
function renderCalendarVacaciones() {
  ...
  while (cursor <= gridEnd) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (personas.some(p => p.consumeVacaciones === false)) cls += " no-consume";
...
function openVacacionModal(isoInicio) {
  ...
  vacConsume = true;
  updateVacConsumeButtons();
  updateVacPersonaButtons();
  openModal("overlayVacacion");
}

function updateVacConsumeButtons() {
  document.querySelectorAll("#vacConsumePicker .seg-btn").forEach(btn => {
    btn.classList.toggle("active", (btn.dataset.consume === "si") === vacConsume);
  });
}

document.querySelectorAll("#vacConsumePicker .seg-btn").forEach(btn => {
  btn.onclick = () => { vacConsume = btn.dataset.consume === "si"; updateVacConsumeButtons(); };
});
...
document.getElementById("btnGuardarVacacion").onclick = () => {
  ...
  dbSet(`${ROOT}/vacaciones/${nuevoId}`, { persona: vacSelectedPersona, inicio, fin, consumeVacaciones: vacConsume });
  closeModal("overlayVacacion");
  toast(`Vacaciones de ${NOMBRES[vacSelectedPersona]} guardadas: ${inicio} → ${fin}${vacConsume ? "" : " (no consume)"}`);
...
function renderVacInfoStep() {
  ...
  const consume = entry.consumeVacaciones !== false;
  const row = document.createElement("div");
  ...
  row.innerHTML = `<span><span class="dot" style="background:${VAC_COLORS[entry.persona]};display:inline-block;width:9px;height:9px;border-radius:2px;margin-right:6px;"></span>
    ${NOMBRES[entry.persona]} · ${entry.inicio} → ${entry.fin}
    <span class="muted" style="display:block;margin-top:4px;">${consume ? "Consume vacaciones" : "No consume (miércoles cambiado)"}</span></span>`;
  list.appendChild(row);

  const btnToggleConsume = document.createElement("button");
  btnToggleConsume.className = "btn ghost";
  btnToggleConsume.style.width = "100%";
  btnToggleConsume.style.marginBottom = "8px";
  btnToggleConsume.textContent = consume ? "Marcar TODO el rango como 'no consume'" : "Marcar TODO el rango como 'consume'";
  btnToggleConsume.onclick = () => {
    dbUpdate(`${ROOT}/vacaciones/${entry.id}`, { consumeVacaciones: !consume });
    toast(`Actualizado: todo el rango ahora ${consume ? "no consume" : "consume"} vacaciones.`);
    closeModal("overlayVacacionInfo");
  };
  list.appendChild(btnToggleConsume);

  if (entry.inicio !== entry.fin) {
    const btnToggleDia = document.createElement("button");
    btnToggleDia.className = "btn ghost";
    btnToggleDia.style.width = "100%";
    btnToggleDia.style.marginBottom = "8px";
    btnToggleDia.textContent = `Cambiar solo el ${formatShort(parseISO(vacInfoDiaISO))} a '${consume ? "no consume" : "consume"}'`;
    btnToggleDia.onclick = () => {
      cambiarConsumoUnDia(entry.id, vacInfoDiaISO);
      closeModal("overlayVacacionInfo");
    };
    list.appendChild(btnToggleDia);
  }
...
function quitarUnDiaVacacion(rangeId, iso) {
  ...
  if (entry.inicio === entry.fin) {
    ...
    dbUpdate(`${ROOT}/vacaciones/${rangeId}`, { fin: dstr(addDays(parseISO(iso), -1)) });
    const nuevoId = db.ref(`${ROOT}/vacaciones`).push().key;
    dbSet(`${ROOT}/vacaciones/${nuevoId}`, {
      persona: entry.persona,
      inicio: dstr(addDays(parseISO(iso), 1)),
      fin: entry.fin,
      consumeVacaciones: entry.consumeVacaciones !== false
    });
    toast("Día eliminado; el rango se ha dividido en dos.");
  }
}

function cambiarConsumoUnDia(rangeId, iso) {
  const entry = state.vacaciones[rangeId];
  if (!entry) return;

  const consumeOriginal = entry.consumeVacaciones !== false;
  const nuevoConsume = !consumeOriginal;

  if (entry.inicio === entry.fin) {
    dbUpdate(`${ROOT}/vacaciones/${rangeId}`, { consumeVacaciones: nuevoConsume });
    toast(`Día actualizado: ahora ${nuevoConsume ? "consume" : "no consume"} vacaciones.`);
    return;
  }
  ...
  if (iso === entry.inicio) {
    ...
    dbUpdate(`${ROOT}/vacaciones/${rangeId}`, { fin: dstr(addDays(parseISO(iso), -1)) });
    const nuevoIdResto = db.ref(`${ROOT}/vacaciones`).push().key;
    dbSet(`${ROOT}/vacaciones/${nuevoIdResto}`, {
      persona: entry.persona,
      inicio: dstr(addDays(parseISO(iso), 1)),
      fin: entry.fin,
      consumeVacaciones: consumeOriginal
    });
  }

  const nuevoIdDia = db.ref(`${ROOT}/vacaciones`).push().key;
  dbSet(`${ROOT}/vacaciones/${nuevoIdDia}`, {
    persona: entry.persona,
    inicio: iso,
    fin: iso,
    consumeVacaciones: nuevoConsume
  });

  toast(`El ${formatShort(parseISO(iso))} ahora ${nuevoConsume ? "consume" : "no consume"} vacaciones; el resto del rango sigue igual.`);
}
...
function renderResumenVacaciones() {
  ...
  const hoy = todayStr();
  const consumidos = { B: 0, BF: 0, J: 0, JC: 0 };
  const previstos = { B: 0, BF: 0, J: 0, JC: 0 };

  Object.values(state.vacaciones).forEach(v => {
    if (v.consumeVacaciones === false) return;
    let d = parseISO(v.inicio);
    const fin = parseISO(v.fin);
    while (d <= fin) {
      const iso = dstr(d);
      if (d.getFullYear() === anioActual) {
        previstos[v.persona] = (previstos[v.persona] || 0) + 1;
        if (iso <= hoy) consumidos[v.persona] = (consumidos[v.persona] || 0) + 1;
      }
      d = addDays(d, 1);
    }
  });
  ...
  wrap.innerHTML = LETTERS.map(l => `
    <div class="resumen-row">
      ...
      <span class="days">${consumidos[l]} consumidos · ${previstos[l]} previstos</span>
    </div>
  `).join("");
}
