...
function creditarGuardiaBF(weekKey) {
  db.ref(`${ROOT}/semanasAcreditadas/${weekKey}`).transaction((cur) => {
    ...
    if (!error && committed) {
      db.ref(`${ROOT}/bolsaHoras`).transaction((cur) => (cur || 0) + 8);
  ...
  while (cursor <= gridEnd) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (entry && entry.horas) {
        ...
        if (entry.concepto) {
          cell.innerHTML += `<div class="concepto" title="${entry.concepto}">${entry.concepto}</div>`;
        }
      }
...
document.getElementById("btnGuardarHoras").onclick = () => {
  ...
  db.ref(`${ROOT}/horasDias/${selectedDayISO}`).set(registro);
  db.ref(`${ROOT}/bolsaHoras`).transaction((cur) => (cur || 0) + delta);
...
document.getElementById("btnBorrarHoras").onclick = () => {
  ...
  if (anterior) {
    ...
    db.ref(`${ROOT}/horasDias/${selectedDayISO}`).remove();
    db.ref(`${ROOT}/bolsaHoras`).transaction((cur) => (cur || 0) - deltaAnterior);
  }
  closeModal("overlayDia");
};
...
document.querySelectorAll("#letterPicker button").forEach(btn => {
  btn.onclick = () => {
    ...
    if (btn.dataset.letter === "") db.ref(`${ROOT}/guardOverrides/${key}`).remove();
    else db.ref(`${ROOT}/guardOverrides/${key}`).set(btn.dataset.letter);
...
document.getElementById("btnGuardarPauta").onclick = () => {
  ...
  db.ref(`${ROOT}/patronGuardia`).set(pautaEdit);
  db.ref(`${ROOT}/patronInicio`).set(inicioLunes);
...
function marcarCanje(iso) {
  db.ref(`${ROOT}/miercolesCanjeados/${iso}`).set(true);
  db.ref(`${ROOT}/bolsaHoras`).transaction((cur) => (cur || 0) - 8);
...
document.getElementById("btnDeshacerSi").onclick = () => {
  db.ref(`${ROOT}/miercolesCanjeados/${deshacerISO}`).remove();
  db.ref(`${ROOT}/bolsaHoras`).transaction((cur) => (cur || 0) + 8);
...
document.getElementById("btnGuardarVacacion").onclick = () => {
  ...
  db.ref(`${ROOT}/vacaciones/${nuevoId}`).set({ persona: vacSelectedPersona, inicio, fin });
...
function comprobarConflictoGuardia(persona, inicio, fin) {
  ...
  list.querySelectorAll("button[data-letter]").forEach(btn => {
    btn.onclick = () => {
      db.ref(`${ROOT}/guardOverrides/${btn.dataset.monday}`).set(btn.dataset.letter);
...
function openVacInfoModal(iso, personas) {
  ...
  personas.forEach(p => {
    ...
    delBtn.onclick = () => {
      db.ref(`${ROOT}/vacaciones/${p.id}`).remove();
